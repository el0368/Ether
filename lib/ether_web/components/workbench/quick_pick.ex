defmodule EtherWeb.Workbench.QuickPick do
  ...
  def update(assigns, socket) do
    ...
    initial_items = 
      case mode do
        ...
        "files" -> [] # Wait for input or show recents?
        _ -> []
      end
  ...
  def handle_event("execute_command", %{"id" => id}, socket) do
    if socket.assigns.mode == "files" do
      ...
      send(self(), {:close_quick_pick, nil}) # Close explicitly
    else
  ...
  defp default_commands do
    [
      %{id: "show_commands", label: "Show All Commands", description: "Ctrl+Shift+P", active: true},
      %{id: "go_to_file", label: "Go to File...", description: "Ctrl+P", active: false},
      %{id: "toggle_sidebar", label: "View: Toggle Primary Side Bar Visibility", description: "Ctrl+B", active: false}
    ]
  end
  ...
  defp search_files(query) do
    case Ether.Agents.FileServerAgent.find_files(query) do
      {:ok, files} -> files |> Enum.take(20) # Limit to 20
      _ -> []
    end
  end
end
