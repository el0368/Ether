  ...
  defp safe_copy(src, dest) do
    ...
    case File.cp(src, dest) do
      ...
      {:error, :eacces} ->
        ...
        old_path = "#{dest}.old"
        # Try to remove previous old file
        File.rm(old_path)
        ...
        case File.rename(dest, old_path) do
          :ok ->
            ...
            Mix.shell().info(
              "    (Shadow-Copied): Replaced in-use file #{Path.basename(dest)}. Change will apply on next restart."
            )

          {:error, _} ->
            Mix.shell().error(
              "    Permission denied: #{Path.relative_to_cwd(dest)} is locked and cannot be renamed."
            )
        end
