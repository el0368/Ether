...
const beam = @import("beam/beam.zig");
const e = @import("beam/erl_nif.zig");

// Re-export ErlNifEnv and ERL_NIF_TERM for entry.c compatibility
pub const ErlNifEnv = e.ErlNifEnv;
pub const ERL_NIF_TERM = e.ERL_NIF_TERM;

// API Struct matching C definition (The Linker Shield)
const WinNifApi = extern struct {
    make_badarg: *const fn (env: ?*ErlNifEnv) callconv(.c) ERL_NIF_TERM,
    inspect_binary: *const fn (env: ?*ErlNifEnv, term: ERL_NIF_TERM, bin: *e.ErlNifBinary) callconv(.c) c_int,
    ...
    make_binary: *const fn (env: ?*ErlNifEnv, bin: *e.ErlNifBinary) callconv(.c) ERL_NIF_TERM,
    alloc_binary: *const fn (size: usize, bin: *e.ErlNifBinary) callconv(.c) c_int,
    release_binary: *const fn (bin: *e.ErlNifBinary) callconv(.c) void,
...
fn flush_buffer(ctx: *ScanContext) void {
    ...
    // Send chunk
    send_chunk(ctx.env, ctx.api, ctx.pid, ctx.buffer);
    ctx.buffer.clearRetainingCapacity();
    ctx.entry_count = 0;
}

fn send_chunk(env: ?*ErlNifEnv, api: *const WinNifApi, pid: *const anyopaque, buffer: *std.ArrayListUnmanaged(u8)) void {
    var bin: e.ErlNifBinary = undefined;
    if (api.alloc_binary(buffer.items.len, &bin) == 0) return;
    @memcpy(bin.data[0..bin.size], buffer.items);

    // Msg = {:binary, Binary}
    // Using Level 5 style Marshalling (Simulated via API)
...
export fn zig_scan(env: ?*ErlNifEnv, argc: c_int, argv: [*c]const ERL_NIF_TERM, api: *const WinNifApi) ERL_NIF_TERM {
    ...
    // 1. Parse Path
    var path_bin: e.ErlNifBinary = undefined;
